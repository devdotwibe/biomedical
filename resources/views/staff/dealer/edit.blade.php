  ...
  <div class="row">
    ...
    <div class="col-md-4 col-md-offset-2">
            ...
            <div class="profileId">
                ...
                <div class="profileOverlay">
                    <div class="imageupload_sec" style="display:none;">
                        <form id="frofile_form" method="post" enctype="multipart/form-data" action="{{ route('admin.dealer.upload',$dealer->dealer_id) }}">
                            @csrf       
                            {{method_field('PUT')}}         
        ...
        <div class="box-body">
            ...
            <strong><i class="fas fa-map-marker-alt mr-1"></i> Verification Status </strong> @if($dealer->status!="Y") <button type="button" class="label label-success" onclick="verifydealer('{{route('admin.dealer.verify',$dealer->dealer_id)}}')"> <i class="fa fa-check"></i> verify</button> @endif
            @if($dealer->status=="Y")
        ...
        <div class="col-md-4">
            <div class="box">                    
                <div class="box-body">
                    <div class="form">
                        <form method="POST" id="profile-form"action="{{ route('dealer.update', $dealer->dealer_id) }}" enctype="multipart/form-data">
                        @csrf

               {{method_field('PUT')}}
                            <div class="form-group">
                                <label for="name"> Organization Name *</label>
                                <input type="text" name="name" id="name" class="form-control" value="{{old('name',$dealer->dealer_name)}}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="phone" id="phone" class="form-control" value="{{old('phone',$dealer->phone)}}">
                                <p class="error-content alert-danger">
                                    {{ $errors->first('phone') }}
                                </p>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="text" name="alter_phone" id="alter_phone" class="form-control" value="{{old('alter_phone',$dealer->alter_phone)}}">
                                <p class="error-content alert-danger">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="email" id="email" class="form-control" value="{{old('email',$dealer->email)}}">
                                <p class="error-content alert-danger">
                                    {{ $errors->first('email') }}
                                </p>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="text" name="address" id="address" class="form-control" rows=5 value="{{old('address',$dealer->address)}}">
                                <p class="error-content alert-danger">
                            ...
                            <div class="form-group">
                                <button type="submit" class="btn btn-success">Save <i class="fa fa-save fa-sm"></i></button> &nbsp;&nbsp; <button type="button" class="btn" onclick="window.location.href='{{ route('dealer.index') }}'">Back to Dealer <i class="fa fa-times fa-sm"></i></button>
                            </div>
...
<script type="text/javascript">
    ...
    $(".del_btn").click(function () {
		...
		$.ajax({
			type:'POST',
			url:'{{ route('dealer.profile.update', $dealer->dealer_id) }}',
			 ...
			 if ($(this).val()) {                 
                ...
                img.onload = function() {
                    ...
                    if(fileSize > 512000){ 
                        ...
                        var url = "{{ route('admin.dealer.upload',$dealer->dealer_id) }}";
                        $.ajax({
                            type: "POST",
                            cache: false,
                            processData: false,
