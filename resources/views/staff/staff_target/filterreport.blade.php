...
@section('title', 'Staff Sales Target Report')

@section('content')

<section class="content-header">
  <h1>
    Staff Sales Target Report - {{$year}}
  </h1>
  <ol class="breadcrumb">
    <li><a href="<?php echo URL::to('staff'); ?>"><i class="fa fa-dashboard"></i> Home</a></li>
    <li class="active">Staff Sales Target Report- {{$year}}</li>
  </ol>
</section>
      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">              
              <table id="staffTargetTable" class="table table-bordered table-striped" style="width: 100%">
                <thead>
                  <tr>
                    <th>Sales</th>
                    @for ($i = $mstart*1; $i <= $mend*1; $i++)
                      <th>{{\Carbon\Carbon::parse(sprintf("%02d-%02d-01",$year,$i))->format('M')}}</th>
                    @endfor
                    <th>Total</th>
                  </tr>
                </thead>
                ...
                <tbody id="staff-group-{{$staff->id}}" data-staff="{{$staff->id}}" class="collapse-body">
                    <tr class="staff-main-{{$staff->id}} collapse-parent" data-staff="{{$staff->id}}" style="display: none" >
                        <th> <a  class="staff-{{$staff->id}} collapse-staff" data-staff="{{$staff->id}}" onclick="togglestaff({{$staff->id}})"> {{$staff->name}} </a> </th>
                        
                      @for ($i = $mstart*1; $i <= $mend*1; $i++)
                        @php
                            $nm=\Carbon\Carbon::parse(sprintf("%02d-%02d-01",$year,$i))->format('M');
                        @endphp
                        <th id="staff-{{$staff->id}}-{{strtolower($nm)}}">{{$staff->nm}}</th>
                      @endfor
                    ...
                    @php
                        ...
                        $equipments_brand=\App\Models\StaffTarget::where("target_year",$year)->whereBetween("target_month",[$mstart,$mend])->where('sale_product_type','equipments')->where('staff_id',$staff->id);
                        $accessories_brand=\App\Models\StaffTarget::where("target_year",$year)->whereBetween("target_month",[$mstart,$mend])->where('sale_product_type','accessories')->where('staff_id',$staff->id);
                    ...
                    $equipments_brand_ids=$equipments_brand->groupBy('brand_id')->pluck('brand_id')->all();
                    $accessories_brand_ids=$accessories_brand->groupBy('brand_id')->pluck('brand_id')->all();
                    ...
                    $strg4="";
                    $strg5="";
                    foreach ($equipments_brand_ids as $brand_id) {
                        $brand=\App\Brand::find($brand_id);

                        $strg1.='
                        <td  class="text-center">'.$brand->name.' 
                            <a onclick="removebrandtarget('.$staff->id."','equipments',".$brand->id.')" class="fa fa-times text-danger" data-toggle="tooltip" data-placement="top" title="Delete Brand"><a>
                            <input type="hidden" name="equipments_'.$staff->id.'_brand[]" value="'.$brand->id.'" class="equipments_'.$staff->id.'_brand_target">
                        </td>';
                        $strg2.='<td  class="text-center"><a onclick="addbrandmonth('.$staff->id.",'equipments',".$brand->id.')">'.(\App\Models\StaffTarget::where('sale_product_type','equipments')->where('staff_id',$staff->id)->where("target_year",$year)->whereBetween("target_month",[$mstart,$mend])->where("brand_id",$brand_id)->sum('target_amount')).'</a></td>';
                        $strg3.='<td  class="text-center">'.(\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year,$mstart,$mend){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse(sprintf("%02d-%02d-01",$year,$mstart*1))->startOfMonth(),\Carbon\Carbon::parse("$year-$mend-31")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->where('brand_id',$brand->id)->pluck('id')->all())->sum('sale_amount')).'</td>';
                            
                        $strg4.='<td  class="text-center"></td>';
                        $strg5.='<td  class="text-center"></td>';
                    }
                    ...
                    foreach ($accessories_brand_ids as $brand_id) {
                        ...
                        $strg6.='
                        <td  class="text-center">'.$brand->name.' 
                            <a onclick="removebrandtarget('.$staff->id."','accessories',".$brand->id.')" class="fa fa-times text-danger" data-toggle="tooltip" data-placement="top" title="Delete Brand"><a>
                            <input type="hidden" name="accessories_'.$staff->id.'_brand[]" value="'.$brand->id.'" class="accessories_'.$staff->id.'_brand_target">
                        </td>';
                        $strg7.='<td  class="text-center"><a onclick="addbrandmonth('.$staff->id.",'accessories',".$brand->id.')">'.(\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->whereBetween("target_month",[$mstart,$mend])->where("brand_id",$brand_id)->sum('target_amount')).'</a></td>';
                        $strg8.='<td  class="text-center">'.(\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year,$mstart,$mend){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse(sprintf("%02d-%02d-01",$year,$mstart*1))->startOfMonth(),\Carbon\Carbon::parse("$year-$mend-31")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->where('brand_id',$brand->id)->pluck('id')->all())->sum('sale_amount')).'</td>';
                            
                        $strg9.='<td  class="text-center"></td>';
                        $strg10.='<td  class="text-center"></td>';
                    }


                    @endphp
                    <tr class="staff-g-{{$staff->id}} collapse-child" data-staff="{{$staff->id}}">
                        <td colspan="{{(intval($mend)-intval($mstart))+3}}"><a  class="staff-{{$staff->id}} collapse-staff" data-staff="{{$staff->id}}" onclick="togglestaff({{$staff->id}})"> {{$staff->name}} </a> </td>
                        <td colspan="{{count($equipments_brand_ids)+1}}" class="text-right"></td>
                    </tr>
                    <tr class="staff-g-{{$staff->id}} collapse-child" data-staff="{{$staff->id}}">
                        <td colspan="{{(intval($mend)-intval($mstart))+3}}"></td>
                        <td colspan="{{count($equipments_brand_ids)+1}}" class="text-center"><h3>Equipments</h3></td>
                    </tr>
                    <tr id="staff-g-{{$staff->id}}-1" class="staff-g-{{$staff->id}} collapse-child" data-staff="{{$staff->id}}">
                        <td colspan="{{(intval($mend)-intval($mstart))+3}}"></td>
                        {!!$strg1!!}
                    </tr>
                    <tr id="staff-g-{{$staff->id}}-2" class="staff-g-{{$staff->id}} collapse-child" data-staff="{{$staff->id}}">
                        <th> <span class="badge btn-block btn-primary">Target</span> </th>
                        @for ($i = $mstart*1; $i <= $mend*1; $i++)
                          <th>{{\App\Models\StaffTarget::where('sale_product_type','equipments')->where('staff_id',$staff->id)->whereIn("brand_id",$equipments_brand_ids)->where("target_year",$year)->where("target_month",$i)->sum('target_amount')}}</th>
                        @endfor
                        <td>{{\App\Models\StaffTarget::where('sale_product_type','equipments')->where('staff_id',$staff->id)->where("target_year",$year)->whereIn("brand_id",$equipments_brand_ids)->whereBetween("target_month",[$mstart,$mend])->sum('target_amount')}}</td>
                        {!!$strg2!!}

                    </tr>
                    <tr id="staff-g-{{$staff->id}}-3" class="staff-g-{{$staff->id}} collapse-child" data.equipments-staff="{{$staff->id}}">
                        <th><span class="badge btn-block btn-warning">Achived</span> </th>
                        @for ($i = $mstart*1; $i <= $mend*1; $i++)
                        <th>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year,$i){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse(sprintf("%02d-%02d-01",$year,$i))->startOfMonth(),\Carbon\Carbon::parse(sprintf("%02d-%02d-01",$year,$i))->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</th>
                        @endfor
                        <td>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year,$mstart,$mend){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse(sprintf("%02d-%02d-01",$year,$mstart))->startOfMonth(),\Carbon\Carbon::parse(sprintf("%02d-%02d-01",$year,$mend))->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</td>
                        {!!$strg3!!}
                        
                    </tr>
                    <tr id="staff-g-{{$staff->id}}-4" class="staff-g-{{$staff->id}} collapse-child" data.equipments-staff="{{$staff->id}}">
                        <th><span class="badge btn-block  btn-danger">Commission</span> </th>
                        @php
                            $commision=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-01-01")->startOfMonth(),\Carbon\Carbon::parse("$year-01-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','01')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','01',{{$commision}})">{{$commision}}</a></th>
                        @php
                            $commision=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-02-01")->startOfMonth(),\Carbon\Carbon::parse("$year-02-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','02')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','02',{{$commision}})">{{$commision}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-03-01")->startOfMonth(),\Carbon\Carbon::parse("$year-03-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','03')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','03',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commision=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-04-01")->startOfMonth(),\Carbon\Carbon::parse("$year-04-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','04')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','04',{{$commision}})">{{$commision}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-05-01")->startOfMonth(),\Carbon\Carbon::parse("$year-05-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','05')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','05',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-06-01")->startOfMonth(),\Carbon\Carbon::parse("$year-06-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','06')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','06',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-07-01")->startOfMonth(),\Carbon\Carbon::parse("$year-07-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','07')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','07',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-08-01")->startOfMonth(),\Carbon\Carbon::parse("$year-08-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','08')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','08',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-09-01")->startOfMonth(),\Carbon\Carbon::parse("$year-09-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','09')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','09',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-10-01")->startOfMonth(),\Carbon\Carbon::parse("$year-10-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','10')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','10',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-11-01")->startOfMonth(),\Carbon\Carbon::parse("$year-11-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','11')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','11',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-12-01")->startOfMonth(),\Carbon\Carbon::parse("$year-12-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','12')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'equipments','12',{{$commission}})">{{$commission}}</a></th>
                        <td>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-01-01")->startOfMonth(),\Carbon\Carbon::parse("$year-12-31")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',1)->whereIn('brand_id',$equipments_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('target_amount')}}</td>
                        {!!$strg4!!}
                        <td colspan="2"></td>
                    </tr>
                    <tr  id="staff-g-{{$staff->id}}-5" class="staff-g-{{$staff->id}} collapse-child" data.equipments-staff="{{$staff->id}}">
                        <th><span class="badge btn-block btn-info">Paid</span> </th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','01')">{{\App\Models\StaffPaidTarget::where('target_month','01')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','02')">{{\App\Models\StaffPaidTarget::where('target_month','02')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','03')">{{\App\Models\StaffPaidTarget::where('target_month','03')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','04')">{{\App\Models\StaffPaidTarget::where('target_month','04')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','05')">{{\App\Models\StaffPaidTarget::where('target_month','05')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','06')">{{\App\Models\StaffPaidTarget::where('target_month','06')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','07')">{{\App\Models\StaffPaidTarget::where('target_month','07')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','08')">{{\App\Models\StaffPaidTarget::where('target_month','08')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','09')">{{\App\Models\StaffPaidTarget::where('target_month','09')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','10')">{{\App\Models\StaffPaidTarget::where('target_month','10')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','11')">{{\App\Models\StaffPaidTarget::where('target_month','11')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'equipments','12')">{{\App\Models\StaffPaidTarget::where('target_month','12')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</a></th>
                        <td>{{\App\Models\StaffPaidTarget::where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','equipments')->sum('paid_amount')}}</td>
                        {!!$strg5!!}
                        <td colspan="2"></td>
                    </tr>
                    ...
                    <tr class="staff-g-{{$staff->id}} collapse-child" data-staff="{{$staff->id}}">
                        <td colspan="{{(intval($mend)-intval($mstart))+3}}"></td>
                        <td colspan="{{count($accessories)+3}}" class="text-right"></td>
                    </tr>
                    <tr class="staff-g-{{$staff->id}} collapse-child" data-staff="{{$staff->id}}">
                        <td colspan="{{(intval($mend)-intval($mstart))+3}}"></td>
                        <td colspan="{{count($accessories)+3}}" class="text-center"><h3>Accessories</h3></td>
                    </tr>
                    <tr id="staff-g-{{$staff->id}}-6" class="staff-g-{{$staff->id}} collapse-child" data-staff="{{$staff->id}}">
                        <td colspan="{{(intval($mend)-intval($mstart))+3}}"></td>
                        {!!$strg6!!}
                    </tr>
                    <tr id="staff-g-{{$staff->id}}-7" class="staff-g-{{$staff->id}} collapse-child" data-staff="{{$staff->id}}">
                        <th> <span class="badge btn-block btn-primary">Target</span> </th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",1)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",2)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",3)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",4)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",5)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",6)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",7)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",8)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",9)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",10)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",11)->sum('target_amount')}}</th>
                        <th>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->where("target_month",12)->sum('target_amount')}}</th>
                        <td>{{\App\Models\StaffTarget::where('sale_product_type','accessories')->where('staff_id',$staff->id)->where("target_year",$year)->sum('target_amount')}}</td>
                        {!!$strg7!!}
                    </tr>
                    <tr id="staff-g-{{$staff->id}}-8" class="staff-g-{{$staff->id}} collapse-child" data.accessories-staff="{{$staff->id}}">
                        <th><span class="badge btn-block btn-warning">Achived</span> </th>
                        <th>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-01-01")->startOfMonth(),\Carbon\Carbon::parse("$year-01-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</th>
                        <th>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-02-01")->startOfMonth(),\Carbon\Carbon::parse("$year-02-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</th>
                        <th>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-03-01")->startOfMonth(),\Carbon\Carbon::parse("$year-03-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</th>
                        <th>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-04-01")->startOfMonth(),\Carbon\Carbon::parse("$year-04-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</th>
                        <th>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-05-01")->startOfMonth(),\Carbon\Carbon::parse("$year-05-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</th>
                        ...
                        <th>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-11-01")->startOfMonth(),\Carbon\Carbon::parse("$year-11-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</th>
                        <th>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-12-01")->startOfMonth(),\Carbon\Carbon::parse("$year-12-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</th>
                        <td>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-01-01")->startOfMonth(),\Carbon\Carbon::parse("$year-12-31")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum('sale_amount')}}</td>
                        {!!$strg8!!}
                        <td colspan="2"><button class="btn btn-danger btn-block"  id="accessories_{{$staff->id}}_period" style="display:none"  onclick="updatedate({{$staff->id}},'accessories')">Save</button></td>

                    </tr>
                    <tr id="staff-g-{{$staff->id}}-9" class="staff-g-{{$staff->id}} collapse-child" data.accessories-staff="{{$staff->id}}">
                        <th><span class="badge btn-block  btn-danger">Commission</span> </th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-01-01")->startOfMonth(),\Carbon\Carbon::parse("$year-01-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','01')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','01',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-02-01")->startOfMonth(),\Carbon\Carbon::parse("$year-02-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','02')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','02',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-03-01")->startOfMonth(),\Carbon\Carbon::parse("$year-03-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','03')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','03',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-04-01")->startOfMonth(),\Carbon\Carbon::parse("$year-04-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','04')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','04',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-05-01")->startOfMonth(),\Carbon\Carbon::parse("$year-05-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','05')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','05',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-06-01")->startOfMonth(),\Carbon\Carbon::parse("$year-06-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','06')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','06',{{$commission}})">{{$commission}}</a></th>
                        @php
                           $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-07-01")->startOfMonth(),\Carbon\Carbon::parse("$year-07-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','07')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount'); 
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','07',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-08-01")->startOfMonth(),\Carbon\Carbon::parse("$year-08-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','08')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','08',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-09-01")->startOfMonth(),\Carbon\Carbon::parse("$year-09-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','09')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','09',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-10-01")->startOfMonth(),\Carbon\Carbon::parse("$year-10-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','10')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','10',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-11-01")->startOfMonth(),\Carbon\Carbon::parse("$year-11-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','11')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','11',{{$commission}})">{{$commission}}</a></th>
                        @php
                            $commission=\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-12-01")->startOfMonth(),\Carbon\Carbon::parse("$year-12-01")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_month','12')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount');
                        @endphp
                        <th><a onclick="addmonthcommission({{$staff->id}},'accessories','12',{{$commission}})">{{$commission}}</a></th>
                        
                        <td>{{\App\Oppertunity_product::whereHas('oppertunity',function($qry)use($staff,$year){ $qry->where('staff_id',$staff->id)->whereBetween("won_date",[\Carbon\Carbon::parse("$year-01-01")->startOfMonth(),\Carbon\Carbon::parse("$year-12-31")->endOfMonth()])->where('deal_stage',8); })->whereIn('product_id',\App\Product::where('category_type_id',3)->whereIn('brand_id',$accessories_brand_ids)->pluck('id')->all())->sum(DB::raw('IFNULL(( SELECT (oppertunity_products.sale_amount* msp.incentive*1)/100 from msp WHERE msp.product_id=oppertunity_products.product_id ORDER BY id DESC LIMIT 1),0)'))+\App\Models\StaffTargetCommission::where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('target_amount')}}</td>
                        {!!$strg9!!}
                        <td colspan="2"></td>
                    </tr>
                    <tr  id="staff-g-{{$staff->id}}-10" class="staff-g-{{$staff->id}} collapse-child" data.accessories-staff="{{$staff->id}}">
                        <th><span class="badge btn-block btn-info">Paid</span> </th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','01')">{{\App\Models\StaffPaidTarget::where('target_month','01')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','02')">{{\App\Models\StaffPaidTarget::where('target_month','02')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','03')">{{\App\Models\StaffPaidTarget::where('target_month','03')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','04')">{{\App\Models\StaffPaidTarget::where('target_month','04')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','05')">{{\App\Models\StaffPaidTarget::where('target_month','05')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','06')">{{\App\Models\StaffPaidTarget::where('target_month','06')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','07')">{{\App\Models\StaffPaidTarget::where('target_month','07')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','08')">{{\App\Models\StaffPaidTarget::where('target_month','08')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','09')">{{\App\Models\StaffPaidTarget::where('target_month','09')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','10')">{{\App\Models\StaffPaidTarget::where('target_month','10')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','11')">{{\App\Models\StaffPaidTarget::where('target_month','11')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <th><a onclick="addmonthpaidtarget({{$staff->id}},'accessories','12')">{{\App\Models\StaffPaidTarget::where('target_month','12')->where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</a></th>
                        <td>{{\App\Models\StaffPaidTarget::where('target_year',$year)->where('staff_id',$staff->id)->where('sale_product_type','accessories')->sum('paid_amount')}}</td>
                        {!!$strg10!!}
                        <td colspan="2"></td>
                    </tr>
                  </tbody>
