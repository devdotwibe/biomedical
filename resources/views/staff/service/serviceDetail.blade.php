  ...
  <div class="manage-service">
      <div class="row">
        ...
        <div class="col-md-12 outer-sect">
          ...
          if(!empty($service->pmDetailsFeedback($service->id)->get()) && count($service->pmDetailsFeedback($service->id)->get()) > 0)
          {
            $feedback = true;
          }
...
@if($staff_id == 55 || $staff_id == 127 || $staff_id == 34 || $staff_id == 56 )
                      @if($service->status == 'Open' || $service->status == 'Pending' )
                            @if(!empty($serviceTaskComment->service_task_status))
                              ...
                              @if( (!empty($service->pmDetailsApproved($service->id))) && ($serviceTaskComment->service_task_status == 'Call Closed' || $serviceTaskComment->service_task_status == 'Visit Closed') )
                                ...
                                @else
                                  ...
                                  <span style="color:red"> Please add the feedback of the customer to complete the service.</span>
      ...
      <!------------------------------------------above for approve button ------------------------------------------------->
            ...
            @foreach ($contractpdt as $k=> $item)
                ...
                <div class="service-btn-left" data-sd="{{$k}}">
                        ...
                        <a class="add-task btn btn-primary" data-title="<b>
                          {{ $service->serviceUser->business_name }} -- {{ $item->equipment->name }}
                          -- {{ $item->under_contract }} -- {{ $item->productMachineStatus->name }} --
                          -- {{ $service->created_at }}
                          </b>" data-item="{{$item->id}}" {{ ($service->status == "Approved") || ($service->status == "Completed") ? 'disabled' : '' }}>Create Visit</a>
                        ...
                        @if($feedback)
                          ...
                          <a class="feedback btn btn-primary" data-item="{{$item->id}}" {{ ($service->status == "Approved") || ($service->status == "Completed") ? 'disabled' : '' }} attr-service_id="{{ $service->id }}" >Add Feedback</a>
                            ...
                            <tbody id="tabledata">
                                ...
                                @if (!empty($pmDates) && is_array($pmDates) && count($pmDates) > 0)
                                    @foreach ($pmDates as $index => $date)
                                          ...
                                          @if ($staff_id == 55 || $staff_id == 127 || $staff_id == 34 || $staff_id == 56 ||$staff_id==(optional($item->productPM("PM".($index+1)))->engineer_name??0))
                                            <tr>
                                                <td class="mobile_view">
                                          ...
                                          @if(!empty($item->productPM("PM".($index+1))->id) && (!empty($item->productPM("PM".($index+1))->pmtask()->get()) && count($item->productPM("PM".($index+1))->pmtask()->get()) > 0)  )
                                                      ...
                                                      @foreach($item->productPM("PM".($index+1))->pmtask()->get() as $task)
                                                          <tr>
                                                              @php
                                                                ...
                                                                $pm_created_at = \Carbon\Carbon::parse($task->service_day)->format('d-m-Y');
          ...
          <h4>Service Details</h4>
            ...
            <table id="cmsTable" class="mobile_view_table table table-bordered table-striped data-">
                <thead>
                    <tr>
                        @if (( $service->service_type !== 2) && ($staff_id == 55 || $staff_id == 127 || $staff_id == 34 || $staff_id == 56 ))
            ...
            @if ($service->service_type == 2 && !empty($service->pmContract))
                ...
                <table id="cmsTable" class="mobile_view_table table table-bordered table-striped data-">
                    ...
                    <tbody id="tabledata">
                        ...
                        @if (!empty($pmDates) && is_array($pmDates) && count($pmDates) > 0)
                            @foreach ($pmDates as $index => $date)
                                @if ($staff_id == 55 || $staff_id == 127 || $staff_id == 34 || $staff_id == 56 ||$staff_id==optional($service->productPM("PM".($index+1)))->engineer_name)
                                <tr>
                                    <td class="mobile_view">
                                        <span> PM {{ $index + 1 }} </span>
                                        <input type="hidden" name="pm[]" value="PM{{ $index + 1 }}">
                ...
                <tbody id="tabledata">
                  @foreach($callResponse as $values)
                        <tr>
                            ...
                            <td class="mobile_view">{{ (!empty($values->taskCommentContactPerson->name)) ? $values->taskCommentContactPerson->name : '' }}</td>
                              @php
                              $callResponse_created = \Carbon\Carbon::parse($values->created_at)->format('d-m-Y');
                              @endphp

                            <td class="mobile_view">{{ $callResponse_created }}</td>
...
@if(!$tasks->isEmpty())
          ...
          <div class="row">
          <h4>Task Details</h4>
          <div class="col-sm-12 service-table-style-2">
            <table id="cmsTable" class="mobile_view_table table table-bordered table-striped data-" >
                <thead>
                  <tr>
                    <th class="col-wide-4">Task Name</th>
                ...
                <tbody id="tabledata">
                    @foreach($tasks as $task)
                        <tr>
                            ...
                            @php
                              $task_created = \Carbon\Carbon::parse($task->created_at)->format('d-m-Y');
                            @endphp

                            <td class="mobile_view">{{ $task_created }}</td>
                            <td class="mobile_view">{{ $task->service_task_status }}</td>
                            @if($staff_id == 127 || $staff_id == 55)
                              @if($task->service_task_status == 'Task Created')
                              <td><a class="btn delete-task" href="{{ route('staff.service-task-delete',$task->id) }}"><img src="{{ asset('images/delete.svg') }}"></a></td>
                ...
                <tbody id="tabledata">
                    @foreach($serviceParts as $servicePart)
                        <tr>
                            ...
                            @php
                              $opp_created = \Carbon\Carbon::parse($servicePart->created_at)->format('d-m-Y');
                            @endphp

                            <td class="mobile_view">{{ $opp_created }}</td>
                ...
                <tbody id="tabledata">
                    @foreach($servicePartsTests as $servicePartTest)
                        <tr>
                            ...
                            <td class="mobile_view">{{ $service->equipment_serial_no }}</td>
                            @php
                            $service_part_created = \Carbon\Carbon::parse($servicePartTest->created_at)->format('d-m-Y');
                            @endphp
                            <td class="mobile_view">{{ $service_part_created }}</td>
                ...
                <tbody id="tabledata">
                    @foreach($serviceOppertunity as $values)
                        <tr>
                            ...
                            @php
                            $service_opp_created = \Carbon\Carbon::parse($values->created_at)->format('d-m-Y');
                            @endphp
                            <td class="mobile_view">{{ $service_opp_created }}</td>
        ...
        <form  id="frm_newVisitTaskform" method="post" action="{{ route('staff.service-newVisitTask') }}" enctype="multipart/form-data">
              ...
              <div class="form-group" id="visit_details">
                      ...
                      <span id="create-Task-title">
                        ...
                        <input type="text" style="z-index:99999 !important; " value="" class="form-control" id="schedule_date" name="schedule_date" placeholder="Schedule Date" readonly><br><br>
...
<script>
      ...
      $('.visitdate-picker').datepicker({
        ...
        function VallidateForm() {
              ...
              console.log('Task Name:', task_name, 'PM No Value:', pm_no_value);
