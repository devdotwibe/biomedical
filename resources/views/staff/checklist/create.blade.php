      ...
      <div class="row">
        ...
        <div class="col-md-9">
          ...
          <div class="box box-primary">
            ...
            <form role="form" name="frm_company" id="frm_company" method="post" action="{{route('checklist.store')}}" enctype="multipart/form-data" >
               @csrf
                <div class="box-body">
              ...
              <div class="box-footer">
                ...
                <button type="button" class="btn btn-danger" onClick="window.location.href='{{route('checklist.index')}}'">Cancel</button>
              </div>
            </form>
          </div>
  ...
  var url = APP_URL+'/admin/change_relative_category';
   $.ajax({
      ...
      function validate_from()
      {
        var name=$("#name").val();
        var related_category_id=$("#related_category_id").val();
        var related_subcategory_id=$("#related_subcategory_id").val();
        ...
        {
          $("#related_category_id_message").show();
        }
        else{
          $("#related_category_id_message").hide();
        }

        if(related_subcategory_id=="")
        {
          $("#related_subcategory_id_message").show();
        }
        else{
          $("#related_subcategory_id_message").hide();
        }

         
        if(name!='' && related_category_id!='' && related_subcategory_id!='')
        {
         $("#frm_company").submit(); 
        }
