       ...
       <div class="col-md-3 leftside-menu">
        ...
        <div class="col-lg-12 margin-tb">
            ...
            <div class="pull-left">
                ...
                <a class="btn btn-sm btn-success" href="{{ route('state.create') }}"> <span class="glyphicon glyphicon-plus"></span>Add State</a>
            ...
            <!-- /.box-header -->
            <div class="box-body">
              <form name="dataForm" id="dataForm" method="post" action="{{ url('/admin/state/deleteAll') }}" />
              @csrf
              ...
              <table id="cmsTable" class="table table-bordered table-striped data-">
                ...
                <tbody>
                    ...
                    <tr id="tr_{{$product->id}}" data-id="{{$product->id}}" data-from ="state">
                        ...
                        <td class="alignCenter">
                            <a class="btn btn-primary btn-xs" href="{{ route('state.edit',$product->id) }}" title="Edit"><span class="glyphicon glyphicon-pencil"></span></a>
                            <?php
                             $state_exit = DB::select('select count(*) as count from  users where `state_id`="'.$product->id.'" ');   
                             if($state_exit[0]->count==0)
                              {
                            ?>
                            <a class="btn btn-danger btn-xs deleteItem" href="{{ route('state.destroy',$product->id) }}" id="deleteItem{{$product->id}}" data-tr="tr_{{$product->id}}" title="Delete"><span class="glyphicon glyphicon-trash"></span></a>
                             <?php
                              }
                             ?>   
                        </td>
