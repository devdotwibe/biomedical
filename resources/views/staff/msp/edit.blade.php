            ...
            <form role="form" name="frm_brand" id="frm_brand" method="post" action="{{ route('staff.brand.update', $brand->id) }}" enctype="multipart/form-data" >
               ...
               {{method_field('PUT')}}
                ...
                <div class="form-group col-md-4 col-sm-6 col-lg-4">
                  ...
                  <input type="text" id="name" name="name" class="form-control" value="{{$brand->name}}" placeholder="Brand Name">
                  <span class="error_message" id="name_message" style="display: none">Field is required</span>
                </div>

                <div class="form-group hideRow col-md-4 col-sm-6 col-lg-4">
                  <label for="image_name">Image*</label>
                  <input type="file" id="image_name" name="image_name" />
                  <input type="hidden" id="current_image" name="current_image" value="<?php echo $brand->image_name ?>"/>

                  <p class="help-block">(Allowed Image Type: jpg,jpeg,png <br />
                    Min. Image size: 50 X 50 pixels)</p>

                    <?php if($brand->image_name != '') { ?>
                  <img src="<?php echo asset("storage/app/public/brand/thumbnail/$brand->image_name") ?>" />
                  <?php } ?>
                ...
                <div class="form-group col-md-4 col-sm-6 col-lg-4">
                  ...
                  <div class="radio">
                    <label>
                      <input type="radio" name="status" id="status1" value="Y" <?php echo ($brand->status == 'Y') ? 'checked': '' ?>>
                      Active
                    </label>
                  </div>
                  <div class="radio">
                    <label>
                      <input type="radio" name="status" id="status1" value="N" <?php echo ($brand->status == 'N') ? 'checked': '' ?>>
                      Inactive
                    </label>
                  </div>
              ...
              <div class="box-footer">
                <button type="submit" class="mdm-btn submit-btn">Submit</button>
                 <button type="button" class="mdm-btn cancel-btn" onClick="window.location.href='{{route('staff.brand.index')}}'">Cancel</button>
              </div>
            </form>
          </div>
