...
<section class="content">
      ...
      <li class="customer_tab_contacts active">
        ...
        <div class="col-md-9">
          ...
          <div class="box box-primary">
            ...
            <div class="pull-left">
              ...
              <form name="dataForm" id="dataForm" method="post" action="{{ url('/staff/contact_person/deleteAll') }}" />
              @csrf
      ...
      <div class="form-group  col-md-6">
        ...
        ?>  
                ...
                <div class="form-group col-md-12">
                  ...
                  <div class="radio">
                    <label>
                      <input type="radio" name="status" id="status_active" value="Y" >
                      Active
                    </label>
                  </div>
                  <div class="radio">
                    <label>
                      <input type="radio" name="status" id="status_deactive" value="N" >
                      Inactive
                    </label>
                  </div>
    ...
    jQuery(document).ready(function() {
       ...
       jQuery(".deletebtn").click(function() {
        ...
        if(contact_type=="HOD")
        {
         ...
         if(status=="Y")
        {
          jQuery("#status_active").attr('checked', true);
        }
        else{
          jQuery("#status_deactive").attr('checked', true);
        }
         $("#email_edit").val(email);
         $("#name_edit").val(name);
...
function validate_contact()
{
 ...
 // alert(contact_type)
  ...
  if(title!=''  && name!=''  && phone!='' && department!='' && designation!='')
        {
          ...
          if(email =='')
          {
              ...
              $('.load-add').show();
              var url = APP_URL+'/staff/add_contact_person';
              $.ajax({
              type: "POST",
              cache: false,
              url: url,
          ...
          else
          {
            var email=$("#email").val();
              ...
              var url = APP_URL+'/staff/emailvalidation';
                      $.ajax({
                      type: "POST",
                      cache: false,
                      url: url,
                      ...
                      success: function (data) {
                            if (data.success === false) {
                                ...
                                $('#email_message').text(data.message).show();
                                $("#email").val('');
                            } else {
                        ...
                        error: function (xhr) {
                            ...
                            if (xhr.responseJSON && xhr.responseJSON.errors && xhr.responseJSON.errors.email) {
                                $('#email_message').text(xhr.responseJSON.errors.email[0]).show();
                            } else {
                                $('#email_message').text('The email must be a valid email address.').show();
                            }
                        }
                    });  
          }
...
function validate_contact_edit()
{
 ...
 var status=$("input[name='status']:checked").val();
  ...
  if(title=="")
      {
        $("#title_edit_message").show();
      }
      else{
        $("#title_edit_message").hide();
      }
     /* if(last_name=="")
      {
        $("last_name_edit_message").show();
      }
      else{
        $("#last_name_edit_message").hide();
      }*/
   if(name=="")
      {
  ...
  if(title!='' && email!='' && name!=''  && phone!='' && department!='' && designation!='')
        {
          ...
          $('.load-edit').show();
          var url = APP_URL+'/staff/contactformedit';
          $.ajax({
          type: "POST",
          cache: false,
          processData: false,
