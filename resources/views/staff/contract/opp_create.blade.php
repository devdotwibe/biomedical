          ...
          @endphp
              ...
              <form method="post" action="{{ route('staff.oppertunity_store') }}" id="contactform" enctype="multipart/form-data">
                  ...
                  <div class="box-body">
                    ...
                    <div class="form-group col-md-2">
                        ...
                        <select name="contract_type" id="contract_type" class="form-control">
                            ...
                            <option value="Warranty" {{ old('contract_type') == 'Warranty' ? 'selected' : '' }} >
                                Warranty</option>
                    ...
                    <table class="table table-bordered opper-table" border="0" style="border-collapse: collapse;">
                        ...
                        @php 
                              ...
                              $total +=$contact_amount_cal;
                                ...
                                $service_tax = $contact_amount_cal * ($service_tax_percentage / 100);


                                $tax_total = $contact_amount_cal + $service_tax;

                                $grand_tax_total += $service_tax;

                                $grand_total = $total + $grand_tax_total;
                        ...
                        <tr>
                            <td >{{ ++$key }}</td>
                            <td ><input type="text" name="equpment_type[]" id="equpment_type{{$key}}"  class="form-control " value="{{ old('equpment_type.'.$key, optional($value->oppertunityProduct)->name) }}" readonly></td>

                            <td >{{optional($value->oppertunityProduct)->category_name}}</td>

                            <td><input type="text" name="equipment_model_no[]" id="equipment_model_no_{{$key}}"  class="form-control " value="{{old('equipment_model_no.'.$key,optional($value->oppertunityProductIb)->equipment_model_no)}}" readonly></td>

                            <td><input type="text" name="equipment_serial_no[]" id="equipment_serial_no_{{$key}}"  class="form-control " value="{{ old('equipment_serial_no.'.$key, optional($value->oppertunityProductIb)->equipment_serial_no) }}" readonly></td>

                            <td>
                                <select class="form-control" name="machine_status_id[]" id="machine_status_id_{{$key}}">
                                    <option value="">-- Select Machine Status --</option>
                                    @foreach ($machine_status as $item)
                                    <option value="{{ $item->id }}"   @if(old('machine_status_id.'.$key) ==$item->id) selected @endif>
                                    
                                       {{ $item->name }}
                                   </option>
                                    @endforeach
                               </select>
                                <span class="error_message" id="machine_status_id_error_{{$key}}" style="display:none"> </span>

                            </td>

                            @php
                                $install_date = optional($value->oppertunityProductIb)->installation_date ? Carbon\Carbon::parse(optional($value->oppertunityProductIb)->installation_date)->format('d-m-Y') : '';
                            @endphp

                            <td><input type="text" name="installation_date[]" id="installation_date_{{$key}}"  class="form-control " value="{{old('installation_date.'.$key,$install_date)}}" readonly></td>

                            <td>
                                <input type="text" id="no_of_pm_{{$key}}" name="no_of_pm[]" value="{{old('no_of_pm',[])[$key]??""}}" data-k="_{{$key}}" data-vk="{{$key-1}}" readonly
                                class="form-control no_of_pm no_of_pm_field" placeholder="No of PMs">
                            ...
                            <td id="before_pm{{$key-1}}">
                                <input type="text" name="revanue[]" value="{{ old('revanue.'.$key)}}" class="form-control revenue_cal" placeholder="Revenue">
                                    <div class="alert alert-danger" id="revanue_error_{{$key}}" style="display:none"> </div>
                            </td>

                            <td >
                                <input type="text" name="amount[]" value="{{ $value->sale_amount}}" class="form-control amount" placeholder=" amount">
                                    <div class="alert alert-danger" id="amount_error_{{$key}}" style="display:none"> </div>
                            </td>

                            <td>
                                <input type="text" name="tax[]" value="{{ $service_tax }}" class="form-control tax" placeholder=" tax" readonly>
                                    <div class="alert alert-danger" id="tax_error_{{$key}}" style="display:none"> </div>
                            </td>

                            <td>
                                <input type="text" name="total[]" value="{{ $tax_total }}" class="form-control total" placeholder=" total" readonly>
                            ...
                            <input type="hidden" name="equipment_id[]" id="equipment_id_{{$key}}"  class="form-control " value="{{old('equipment_id.'.$key,optional($value->oppertunityProductIb)->equipment_id)}}" readonly>
                        
                            <input type="hidden" name="equipment_status_id[]" id="equipment_status_id" value="{{ old('equipment_status_id',optional($value->oppertunityProductIb)->equipment_status_id) }}">

                            <input type="hidden" name="equpment_type[]" id="equpment_type{{$key}}" value="{{ old('equpment_type.'.$key,optional($value->oppertunityProduct)->name) }}">
                            ...
                            <input type="hidden" name="supplay_order[]" id="supplay_order_{{$key}}" value="{{ old('supplay_order.'.$key,optional($value->oppertunityProductIb)->supplay_order) }}">

                            <input type="hidden" name="warraty_expiry_date[]" id="warrenty_end_date_{{$key}}" value="{{ old('warranty_expiry_date.'.$key,optional($value->oppertunityProductIb)->warrenty_end_date) }}">
                     ...
                     @foreach($productDetails as $key=>$value)
                        ...
                        <div class="
                            equipment-details" id="equipment-details-group"  @if(empty(old('equipment_id',optional($value->oppertunityProductIb)->equipment_id))) style="display: none" @endif >
                  ...
                  <div class="box-footer">
                      <button type="submit" id="submit_contract" disabled  class="btn btn-primary">Submit</button>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        function CheckEndDate(element) {
            ...
            const fomatstartDate =  formatYear(startDate);
            const fomatendDate =  formatYear(endDate);

            console.log(fomatstartDate,fomatendDate,'dats');

            const start = new Date(fomatstartDate);
            const end = new Date(fomatendDate);
            if (start >= end) {
        ...
        $(document).on('keyup', '.no_of_pm', function() {
           ...
           $('[name^="no_of_pm"]').each(function() {
            ...
            $('[name^="no_of_pm"]').each(function() {
                ...
                var startvalue = $("#contract_start_date").val();

                var endvalue  = $("#contract_end_date").val();

                var startDate = new Date(formatYear(startvalue));
                var endDate = new Date(formatYear(endvalue));

                var selectstartDate =formatYear(startvalue);
                var selectendDate = formatYear(endvalue);
                ...
                if (selectstartDate !== "" && selectendDate !== "")
                {
                    ...
                    for (var i = 0; i < high_no; i++) {
                        ...
                        var formattedDate = formatYear(formatDate(currentDate));
                        if(i<no && no!=0)
                        {
