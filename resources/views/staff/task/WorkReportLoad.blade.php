      ...
      <div class="datesection" id="datesection">
        <div class="form-group col-md-6">
            ...
            <div class="viewcounts">
                <span>Work ({{ count($attendence) }})</span>
                <span>Leave ({{ $leave }})</span>
                <span>Not Updated ({{ $noupdated_task - $leave }})</span>
            </div>
        </div>
        <br>
        <div class="row month_dispaly">
            ...
            @foreach(range(0, 44) as $i)
                @php
                    ...
                    $a = strtotime($cur_date) >= strtotime($date) ? 1 : 0;
                
                    $status_date = 'calender-'.$i.'-status'; 
                    $work_status = 'calender-'.$i;
                @endphp
    
                <div class="panel panel-default col-md-3 col-sm-6 col-lg-3 " id="{{ $work_status }}" onclick="change_date('{{ $date }}', '{{ $a }}')">
                    <div class="panel-body">
                        {{ \Carbon\Carbon::parse($date)->format('d-m-Y') }}
                        @if(date('D', strtotime($date)) == 'Sun' && ($a ==1) )
                            (Sunday)
                        @endif
                        <br> <span id="{{ $status_date }}"></span>
                    </div>
                </div>

            @endforeach
            <input type="hidden" name="work_date" id="work_date">
            <input type="hidden" name="cur_year" id="cur_year" value="{{ date('Y') }}">
        </div>
    </div>
...
$(function() {
    ...
    var url = APP_URL+'/staff/work/workcalender';
      ...
      success: function (res) {
        if (Array.isArray(res)) {
            $.each(res, function(i, v) {
                ...
                $("#" + v.id).addClass(v.work_status); 
                
                $("#" + v.id + "-status").text(v.date_status);
            });
        } else {
